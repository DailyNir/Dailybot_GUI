...
class CamerasFrame(tkinter.Frame):
    ...
    def subscribe_to_image_topics(self):
            for topic in self.available_image_topics:
                if topic not in self.subscribers:
                    threading.Thread(target=self.subscribe_and_process, args=(topic,)).start()
    def subscribe_and_process(self, topic):
        callback = functools.partial(self.update_image, topic=topic)
    ...
    def create_cameras_view(self):
        ...
        self.scrollable_frame.grid_propagate()

        topics_to_display = self.available_image_topics[:6] + [None] * (6 - len(self.available_image_topics))

        for i, topic in enumerate(topics_to_display):
            ...
            self.topics_option.grid(row=2*(i//3)*2, column=i%3, padx=(20, 10), pady=(20, 10), sticky="w")
            self.take_frame = customtkinter.CTkButton(master=self.scrollable_frame, text="Picture", width=50, fg_color="#2CC985")
            self.take_frame.grid(row=2*(i//3)*2, column=i%3, padx=(20, 10), pady=(20, 10), sticky="e")

            self.image_dict[topic] = tkinter.Label(self.scrollable_frame, width=300, height=300, bg="white")
    ...
    def update_image(self, img_msg, topic):
        try:
            ...
            # Store the image and set the flag
